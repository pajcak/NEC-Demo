...
void Demo::initMonitors() {
	m_controller->addMonitor("127.0.0.1", 12345, 666);
	m_monitors[666] = std::make_pair("127.0.0.1", 12345);
	m_controller->addMonitor("127.0.0.1", 12346, 777);
	m_monitors[777] = std::make_pair("127.0.0.1", 12346);
	m_controller->addMonitor("10.0.0.100", 7142, 1);
	m_monitors[1] = std::make_pair("10.0.0.100", 7142);
	m_controller->addMonitor("10.0.0.101", 7142, 2);
	m_monitors[2] = std::make_pair("10.0.0.101", 7142);
	m_controller->addMonitor("10.0.0.102", 7142, 3);
	m_monitors[3] = std::make_pair("10.0.0.102", 7142);
	m_controller->addMonitor("10.0.0.103", 7142, 4);
	m_monitors[4] = std::make_pair("10.0.0.103", 7142);
	m_controller->addMonitor("10.0.0.104", 7142, 5);
	m_monitors[5] = std::make_pair("10.0.0.104", 7142);
	m_controller->addMonitor("10.0.0.105", 7142, 6);
	m_monitors[6] = std::make_pair("10.0.0.105", 7142);
	m_controller->addMonitor("10.0.0.106", 7142, 7);
	m_monitors[7] = std::make_pair("10.0.0.106", 7142);
	m_controller->addMonitor("10.0.0.107", 7142, 8);
	m_monitors[8] = std::make_pair("10.0.0.107", 7142);
	m_controller->addMonitor("10.0.0.108", 7142, 9);
	m_monitors[9] = std::make_pair("10.0.0.108", 7142);
	m_controller->addMonitor("10.0.0.109", 7142, 10);
	m_monitors[10] = std::make_pair("10.0.0.109", 7142);
	m_controller->addMonitor("10.0.0.110", 7142, 11);
	m_monitors[11] = std::make_pair("10.0.0.110", 7142);
	m_controller->addMonitor("10.0.0.111", 7142, 12);
	m_monitors[12] = std::make_pair("10.0.0.111", 7142);
	m_controller->addMonitor("10.0.0.112", 7142, 13);
	m_monitors[13] = std::make_pair("10.0.0.112", 7142);
	m_controller->addMonitor("10.0.0.113", 7142, 14);
	m_monitors[14] = std::make_pair("10.0.0.113", 7142);
	m_controller->addMonitor("10.0.0.114", 7142, 15);
	m_monitors[15] = std::make_pair("10.0.0.114", 7142);
	m_controller->addMonitor("10.0.0.115", 7142, 16);
	m_monitors[16] = std::make_pair("10.0.0.115", 7142);
	m_controller->addMonitor("10.0.0.116", 7142, 17);
	m_monitors[17] = std::make_pair("10.0.0.116", 7142);
	m_controller->addMonitor("10.0.0.117", 7142, 18);
	m_monitors[18] = std::make_pair("10.0.0.117", 7142);
	m_controller->addMonitor("10.0.0.118", 7142, 19);
	m_monitors[19] = std::make_pair("10.0.0.118", 7142);
	m_controller->addMonitor("10.0.0.119", 7142, 20);
	m_monitors[20] = std::make_pair("10.0.0.119", 7142);
}
    
void Demo::initFunctions() {
	//----------functions-------
    m_initController    =  &CController::initController;
    m_destroyController =  &CController::destroyController;
    m_addMonitor        =  &CController::addMonitor;
    m_deleteMonitor     =  &CController::deleteMonitor;
    m_connectMonitor    =  &CController::connectMonitor;
    m_disconnectMonitor =  &CController::disconnectMonitor;
    m_connectAll        =  &CController::connectAll;
    m_disconnectAll     =  &CController::disconnectAll;
    m_isConnected       =  &CController::isConnected;
	//------param-functions-----
	m_FunctionNames.push_back("Backlight");
	m_GetBacklightFunc = &CController::getBacklight;
	m_SetBacklightFunc = &CController::setBacklight;

	m_FunctionNames.push_back("Contrast");
	m_GetContrastFunc = &CController::getContrast;
	m_SetContrastFunc = &CController::setContrast;

	m_FunctionNames.push_back("Sharpness");
	m_GetSharpnessFunc = &CController::getSharpness;
	m_SetSharpnessFunc = &CController::setSharpness;

	m_FunctionNames.push_back("Brightness");
	m_GetBrightnessFunc = &CController::getBrightness;
	m_SetBrightnessFunc = &CController::setBrightness;

	m_FunctionNames.push_back("Hue");
	m_GetHueFunc = &CController::getHue;
	m_SetHueFunc = &CController::setHue;

	m_FunctionNames.push_back("Paleness");
	m_GetPalenessFunc = &CController::getPaleness;
	m_SetPalenessFunc = &CController::setPaleness;

	m_FunctionNames.push_back("Color temperature");
	m_GetColorTemperatureFunc = &CController::getColorTemperature;
	m_SetColorTemperatureFunc = &CController::setColorTemperature;

	m_FunctionNames.push_back("Gamma correction");
	m_GetGammaCorrectionFunc = &CController::getGammaCorrection;
	m_SetGammaCorrectionFunc = &CController::setGammaCorrection;

	m_FunctionNames.push_back("Volume");
	m_GetVolumeFunc = &CController::getVolume;
	m_SetVolumeFunc = &CController::setVolume;
	//------command-functions-----
	m_CommandNames.push_back("Read power status");
    m_PowerStatusReadFunc = &CController::powerStatusRead;
	m_CommandNames.push_back("Change power status");
    m_PowerControlFunc = &CController::powerControl;
}
...
start:
    ...
    switch (choice) {
		case 0: //ADD monitor
		{
		    ...
		    try {
		    	(m_controller->*m_addMonitor)(addr.c_str(), port, monitorID);
			} catch (char const* s) {
				addDialogue.hideDialogue();
		...
		case 1: //DELETE monitor
		{
		    ...
		    for (vector<pair<int, bool>>::iterator it = monitDialResult.begin(); it != monitDialResult.end(); ++it)
		    {
				if (m_monitors.find(it->first) != m_monitors.end()) {
					...
					try {
						(m_controller->*m_deleteMonitor)(it->first);
					} catch ( char const* s) {
		...
		case 2: //CONNECT monitor
		{
		    ...
		    for (vector<pair<int, bool>>::iterator it = monitDialResult.begin(); it != monitDialResult.end(); ++it)
		    {
				if (m_monitors.find(it->first) != m_monitors.end()) {
					try {
						if (!(m_controller->*m_connectMonitor)(it->first)) {
		...
		case 3: //DISCONNECT monitor
		{
		    ...
		    for (vector<pair<int, bool>>::iterator it = monitDialResult.begin(); it != monitDialResult.end(); ++it)
		    {
				if (m_monitors.find(it->first) != m_monitors.end()) {
					try {
						(m_controller->*m_disconnectMonitor)(it->first);
		...
		case 6: //IS CONNECTED
		{
		    ...
		    for (vector<pair<int, bool>>::iterator it = monitDialResult.begin(); it != monitDialResult.end(); ++it)
		    {
				if (m_monitors.find(it->first) != m_monitors.end()) {
					try {
						if ((m_controller->*m_isConnected)(it->first)) {
		...
		case 7: //GET PARAM
		{
		    ...
		    int (CController::* getterFunction)(int);
		    switch(paramNum) {
		    ...
		    if (m_options[0].second) { //parallelism enabled
				...
				{
					attrArray[i] = new getterThrFuncAttr(this, getterFunction,
														&monitorsDone,
														&printMtx,
														&dialogueMtx,
														monitDialResult[i].first, paramNum);
					
					pthread_create(&threads[i], NULL, (void*(*)(void*))&Demo::helperGetterThrFunc, attrArray[i]);
				}		
				...
				for (vector<pair<int, bool>>::iterator it = monitDialResult.begin(); it != monitDialResult.end(); ++it)
				{
					if (m_monitors.find(it->first) != m_monitors.end()) {
						try {
							...
							int result = (m_controller->*getterFunction)(it->first);
							stringstream ss;
		...
		case 8: //SET PARAM
		{
		    ...
		    void (CController::* setterFunction)(int, int);
		    switch(paramNum) {
		    ...
		    if (m_options[0].second) { //parallelism enabled
				...
				for (int i = 0; i < thrCount; i++)
				{
					attrArray[i] = new setterThrFuncAttr(this, setterFunction,
														&monitorsDone,
														&printMtx,
														&dialogueMtx,
					...
					pthread_create(&threads[i], NULL, (void*(*)(void*))&Demo::helperSetterThrFunc, attrArray[i]);
				}		
				...
				for (vector<pair<int, bool>>::iterator it = monitDialResult.begin(); it != monitDialResult.end(); ++it)
				{
					if (m_monitors.find(it->first) != m_monitors.end()) {
						try {
							...
							(m_controller->*setterFunction)(it->first, paramValue);
							stringstream ss;
							ss << m_FunctionNames[paramNum] << " of monitor "
		    ...
		    if (commandNum == -1) goto start;
		   	...
		   	switch(commandNum) {
							...
							it != monitDialResult.end(); ++it)
					{
						if (m_monitors.find(it->first) != m_monitors.end()) {
							try {
								...
								int result = (m_controller->*m_PowerStatusReadFunc)(it->first);
								stringstream ss;
		   		...
		   		case 1: { //change power status
					...
					if (m_options[0].second) { //parallelism enabled
						...
						for (int i = 0; i < thrCount; i++)
						{
							...
							pthread_create(&threads[i], NULL,
								(void*(*)(void*))&Demo::helperPowerControlThrFunc, attrArray[i]);
						}		
									...
									it != monitDialResult.end(); ++it)
						{
							if (m_monitors.find(it->first) != m_monitors.end()) {
								try {
									...
									(m_controller->*m_PowerControlFunc)(it->first, paramValue);
									stringstream ss;
									ss << "Power status of monitor "
					...
					if (!m_errorFlag_parallel) {
						Dialogue dialS("Change power status", monitorDialogue_X, monitorDialogue_Y, monitorsDone);
						dialS.showDialogue(1, 0);
					}
		   			break;
		   		}
...
void * Demo::helperGetterThrFunc(struct getterThrFuncAttr * attr) {
	attr->thisObj->GetterThrFunc(attr);
	return NULL;
}
...
void Demo::GetterThrFunc (struct getterThrFuncAttr * attr) {
	...
	if (m_monitors.find(attr->monitorID) != m_monitors.end()) {
		try {
			if ( !(m_controller->*m_isConnected)(attr->monitorID) ) {
			...
			int result = (m_controller->*(attr->getterFunc))(attr->monitorID);
			stringstream ss;
...
void * Demo::helperSetterThrFunc(struct setterThrFuncAttr * attr) {
	attr->thisObj->SetterThrFunc(attr);
	return NULL;
}
...
void Demo::SetterThrFunc (struct setterThrFuncAttr * attr) {
	if (m_monitors.find(attr->monitorID) != m_monitors.end()) {
		try {
			...
			(m_controller->*(attr->setterFunc))(attr->monitorID, attr->paramValue);
			stringstream ss;
			ss << m_FunctionNames[attr->paramNum] << " of monitor "
...
void * Demo::helperPowerControlThrFunc(struct powerControlThrFuncAttr * attr) {
	attr->thisObj->PowerControlThrFunc(attr);
	return NULL;
}
...
void Demo::PowerControlThrFunc (struct powerControlThrFuncAttr * attr) {
	if (m_monitors.find(attr->monitorID) != m_monitors.end()) {
		try {
			...
			(m_controller->*m_PowerControlFunc)(attr->monitorID, attr->paramValue);
			stringstream ss;
			ss << "Power status of monitor "
